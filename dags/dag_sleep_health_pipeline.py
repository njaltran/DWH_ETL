...
@dag(
    dag_id='dag_sleep_health_pipeline',
    schedule='@daily',
...
def load_data():
    ...
    from sleep_health_pipeline import sleep_health_source

    # modify the pipeline parameters 
    pipeline = dlt.pipeline(pipeline_name='sleep_health_pipeline',
                     dataset_name='sleep_health',
                     destination='duckdb',
                     full_refresh=False # must be false if we decompose
                     )
    # create the source, the "serialize" decompose option will converts dlt resources into Airflow tasks. use "none" to disable it
    tasks.add_run(pipeline, sleep_health_source(), decompose="serialize", trigger_rule="all_done", retries=0)
